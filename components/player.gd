...
## Two-hand push multiplier (was effectively 2.0) — lower to reduce launch spikes.
@export var dual_push_multiplier : float = 1.55
## Hard cap for launch speed generated by floor pushes.
@export var ground_launch_speed_cap : float = 14.5
## Edge assist upward component when a hand push has no direct ray hit.
@export var edge_assist_upward : float = 0.78
## Edge assist forward component to preserve directional intent.
@export var edge_assist_forward : float = 0.36
...
func _handle_push() -> void:
	...
	if Input.is_action_just_pressed("push_left") and not _left_consumed:
		if left_hand_state == HandState.FREE and _left_cooldown <= 0.0:
			if left_hand_ray.is_colliding() and not left_hand_ray.get_collider() is Vine:
	...
	if Input.is_action_just_pressed("push_right") and not _right_consumed:
		if right_hand_state == HandState.FREE and _right_cooldown <= 0.0:
			if right_hand_ray.is_colliding() and not right_hand_ray.get_collider() is Vine:
	...
	for d in push_dirs:
		combined += d
	var hand_mult : float = 1.0 if push_dirs.size() <= 1 else dual_push_multiplier
	velocity += combined.normalized() * push_force * hand_mult
	if is_on_floor() and velocity.length() > ground_launch_speed_cap:
		velocity = velocity.normalized() * ground_launch_speed_cap
...
func _grab_twig(twig: Node, is_left: bool, hit_point: Vector3) -> void:
	var pivot : Vector3 = twig.call("grab_pos")
	var rope_len : float = maxf((global_position - pivot).length(), 0.5)
	if is_left:
		left_hand_state    = HandState.GRABBING
		left_grab_point    = null
...
func _release_hand(is_left: bool) -> void:
	...
	if not other_grabbing and released_dynamic:
		...
		if is_left:
			if _left_zipline:
				carry_speed = absf(_left_zip_vel) * release_boost_mult
			elif _left_twig:
				carry_speed = maxf(Vector2(velocity.x, velocity.z).length() * 0.65, 6.0)
		else:
			if _right_zipline:
				carry_speed = absf(_right_zip_vel) * release_boost_mult
			elif _right_twig:
				carry_speed = maxf(Vector2(velocity.x, velocity.z).length() * 0.65, 6.0)
		var look_dir := (-head.global_transform.basis.z).normalized()
		var dynamic_cap : float = base_release_cap * (1.12 + float(_combo) * combo_speed_bonus)
		velocity = look_dir * clampf(maxf(carry_speed, 7.5), 7.5, dynamic_cap)
		_fov_pulse = clampf(carry_speed * 1.8, 0.0, 18.0)
		...
		var look_dir := (-head.global_transform.basis.z).normalized()
		var release_spd := base_release_cap * (1.0 + float(_combo) * combo_speed_bonus)
...
func _can_use_edge_push_assist() -> bool:
	if not is_on_floor() or _is_grabbing():
		return false
	return true


func _edge_assist_push_dir(ray: RayCast3D) -> Vector3:
	var base_dir := _push_dir_from(ray)
	var look_fwd := -head.global_transform.basis.z
	look_fwd.y = 0.0
	if look_fwd.length_squared() > 0.001:
		look_fwd = look_fwd.normalized()
	var blended := base_dir * (1.0 - edge_assist_forward) + look_fwd * edge_assist_forward + Vector3.UP * edge_assist_upward
...
func _check_player_collisions() -> void:
	...
	for i in get_slide_collision_count():
		...
		if _active_buff == "Repulsor":
			var stacks := int(_effect_stacks.get("Repulsor", 1))
			var repulse_mult := blue_repulsion_force_mult + float(maxi(stacks - 1, 0)) * 0.20
			# Extra value in Tag/Bomb Tag when colliding with IT.
			if _is_tag_it_peer(other.get_multiplayer_authority()):
				repulse_mult *= 1.12
			bump_force *= repulse_mult
...
func _handle_wind_dash() -> void:
	...
	# Briefly "catch the wind" before the burst.
	velocity *= 0.18
...
func _apply_attraction(delta: float) -> void:
	...
	var pull_speed := ATTRACTION_PULL_SPEED + float(maxi(stacks - 1, 0)) * 1.2
	var pull_radius := ATTRACTION_RADIUS + float(maxi(stacks - 1, 0)) * 0.65
